...
import { motion } from "framer-motion";
import "./EventHighlights.css"; // Imported standard CSS stylesheet
    ...
    <section className="highlights-section">
      ...
      <div className="text-center section-header">
        <motion.p
          ...
          whileInView={{ opacity: 1 }}
          viewport={{ once: true }}
          className="font-orbitron sub-title text-red-500 uppercase"
        >
          What Awaits You
        </motion.p>
        <motion.h2
          ...
          viewport={{ once: true }}
          transition={{ delay: 0.1 }}
          className="font-orbitron main-title text-white"
        >
          RACE DAY <span className="text-gradient-red">LINEUP</span>
        </motion.h2>
        <div className="track-line w-40 mx-auto mt-5" />
      </div>
      ...
      >
        {highlights.map((h, i) => (
          <motion.div
            ...
            variants={item}
            whileHover={{ scale: 1.04, y: -4 }}
            className="glass-card race-border highlight-card text-center"
          >
            <div className="card-icon">{h.icon}</div>
            <h3 className="font-orbitron card-title text-white">
              {h.title}
            </h3>
            <p className="font-rajdhani card-desc text-gray-400">{h.desc}</p>
          </motion.div>
        ))}
      </motion.div>
    </section>
