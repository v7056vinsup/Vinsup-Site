...
import { useState, useEffect } from "react";
import { motion } from "framer-motion";
import "./CountdownTimer.css"; // Imported standard CSS stylesheet
...
function TimeBlock({ value, label }) {
  return (
    <div className="time-block-wrapper">
      ...
      >
        <div className="glass-card race-border time-box glow-red">
          <span className="font-orbitron time-value text-white">
            {String(value).padStart(2, "0")}
          </span>
        </div>
        {/* Separator line */}
        <div className="box-splitter" />
      </motion.div>
      <span className="font-rajdhani time-label text-gray-400 uppercase">
        {label}
      </span>
    </div>
  );
...
export default function CountdownTimer() {
  ...
  return (
    <div className="timer-container">
      <TimeBlock value={timeLeft.days ?? 0} label="Days" />
      <span className="font-orbitron timer-colon text-red-500">:</span>
      <TimeBlock value={timeLeft.hours ?? 0} label="Hours" />
      <span className="font-orbitron timer-colon text-red-500">:</span>
      <TimeBlock value={timeLeft.minutes ?? 0} label="Mins" />
      <span className="font-orbitron timer-colon text-red-500">:</span>
