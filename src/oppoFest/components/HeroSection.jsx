...
//               }`}
//             >
//               <img src={logo} alt="Pass" className="w-36 h-auto object-contain" />
//             </div>
...
import logo from "../../assets/logo.png";
import "./HeroSection.css"; // Imported custom normal CSS stylesheet
    ...
    <Float speed={2} rotationIntensity={1.2} floatIntensity={1.8} floatingRange={[-0.3, 0.3]}>
      <group position={position}>
        <Html transform distanceFactor={distanceFactor} center prepend zIndexRange={[10, 0]}>
          <div
            style={{ width: cardWidth }}
            className={`ticket-card-3d backdrop-blur-xl ${
              color === "gold" ? "gold-variant" : "red-variant"
            }`}
          >
            {/* Header */}
            <div className={`ticket-header ${color === "gold" ? "gold-grad" : "red-grad"}`}>
              <img src={logo} alt="Pass" className="logo-img" />
            </div>

            {/* Body */}
            <div className="ticket-body">
              <p className={`ticket-subtitle uppercase font-rajdhani ${color === "gold" ? "text-yellow-400" : "text-red-400"}`}>
                OPPO SPEED FEST
              </p>
              <h3 className="text-white ticket-title font-orbitron">{title}</h3>

              <div className="ticket-details text-gray-300 font-rajdhani">
                <p>📅 28 June 2026</p>
                <p>⏰ 1PM – 6PM</p>
                <p>📍 Kari Motor Speedway, CBE</p>
              </div>

              <div className={`ticket-footer ${color === "gold" ? "gold-border-t" : "red-border-t"}`}>
                <p className="text-gray-400 price-label font-rajdhani">Ticket Price</p>
                <div className="price-row font-rajdhani">
                  <p className="text-white current-price font-orbitron">₹{price}</p>
                  <p className="text-gray-600 line-through original-price-slashed">₹999</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="hero-content-container">
        <div className="hero-grid">
          ...
          <div className="left-column">
            ...
            >
              <div className="checker-bg-box checker-bg" />
              <span className="font-orbitron tracker-title text-red-400 uppercase">
                TCM Tribe Presents
              </span>
            </motion.div>
            ...
            <motion.h1
              ...
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.35, duration: 0.7 }}
              className="font-orbitron hero-main-title"
            >
              <span className="title-row-1 text-white drop-shadow-2xl">OPPO</span>
              <span className="title-row-2 text-gradient-red drop-shadow-2xl">SPEED</span>
              <span className="title-row-2 text-white drop-shadow-2xl">FEST</span>
            </motion.h1>
            ...
            <motion.p
              ...
              animate={{ opacity: 1 }}
              transition={{ delay: 0.7 }}
              className="font-rajdhani hero-tagline text-gray-300 uppercase"
            >
              SPEED. POWER. PASSION. — ALL ON TRACK!
            </motion.p>
            ...
            <button
              onClick={() => setShowPassModal(true)}
              className="btn-race font-orbitron animate-pulse-red"
            ...
            className="btn-contact font-orbitron animate-pulse-yellow"
          >
            📍 CONTACT US
          </button>
            ...
            >
              {[
                ...
                <div key={i} className="glass-chip glass-card race-border font-rajdhani text-white">
                  <span>{chip.icon}</span>
                  <span>{chip.text}</span>
                </div>
              ))}
            ...
            >
              <p className="font-orbitron timer-label text-gray-500 uppercase">Race Starts In</p>
              <CountdownTimer />
            </motion.div>
            ...
            >
              <button
                onClick={onBookClick}
                className="btn-race large-btn font-orbitron animate-pulse-red"
              >
                🎟️ Students ₹99 · Visitor ₹249
              </button>
              ...
              <a
                href="#highlights"
                className="btn-gold font-orbitron"
              >
                Explore Event
              </a>
            </motion.div>
            ...
            <motion.div
              ...
              className="mt-6"
            >
              <div className="glass-card race-border inline-price-badge glow-red text-center">
                <div>
                  <span className="font-rajdhani text-gray-400 badge-unit-title">Students</span>
                  <span className="font-orbitron badge-unit-price text-white">₹99</span>
                </div>
                <div className="vertical-divider" />
                <div>
                  <span className="font-rajdhani text-gray-400 badge-unit-title">Visitor</span>
                  <span className="font-orbitron badge-unit-price text-white">₹249</span>
                </div>
                <div className="vertical-divider" />
                <span className="font-rajdhani text-gray-500 line-through" style={{ fontSize: '0.875rem' }}>₹999</span>
              </div>
            </motion.div>
          </div>

          {/* ── Right: Floating Three.js Canvas Container ── */}
          <motion.div
            initial={{ opacity: 0, y: 50 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 1, delay: 0.8 }}
